...
def validate_args(args, defaults={}):
    ...
    if args.dynamic_context_parallel:
        assert (
            not args.pipeline_model_parallel_size > 1
        ), 'Dynamic context parallelism not supported with pipeline parallelism'
        assert (
    ...
    if (
        ...
        assert (
            args.moe_pad_experts_for_cuda_graph_inference
    ...
    if args.optimizer not in ('sgd', 'adam'):
        ...
        assert (
            args.experimental_attention_variant is None
